<script>
	...
	async function handleDeleteSection(event, section) {
		event.stopPropagation();
		
		if (!confirm(`Are you sure you want to delete section "${section.SectionName}"? This will also remove all subject assignments and student enrollments for this section.`)) {
			return;
		}
		...
		try {
			...
			const response = await fetch("?/deleteSection", {
				method: "POST",
				body: formData,
			});
			...
			if (result.type === "success") {
				// Reset selected section if it was the deleted one
				...
				alert(result.data?.error || "Failed to delete section");
			}
		} catch (error) {
			...
			<form
				...
				use:enhance={() => {
					...
					return async ({ result }) => {
						...
						if (result.type === "success") {
							closeCreateModal();
							const newSectionId = result.data?.sectionId;
							await invalidateAll();
							if (newSectionId) {
								selectedSection = sections.find(s => s.SectionID === newSectionId) || { SectionID: newSectionId, SectionName: result.data?.sectionName || '' };
								// Subjects are now auto-added, just refresh the section subjects list
								await loadSectionSubjects(newSectionId);
							}
						}
					};
				}}
...
<style>
	...
	@media (prefers-color-scheme: dark) {
		...
		.section-card-item .s-8YsCOcaNWg5j{
			color: #000;
		}
